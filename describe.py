...
def describe(df, ignore_index=False):
    ...
    for feature in numeric_col_names:
        mean_count = mean_column(df[feature].dropna())
        std = std_column(df[feature].dropna())
        min_max = min_max_column(df[feature].dropna())
        per = percentiles_column(df[feature].dropna())
...
#         min_max = min_max_column(df[feature].dropna())
#         per = percentiles_column(df[feature].dropna())
#         desc_dict[feature] = [mean_count[1], mean_count[0], std[0], min_max[0], per[0], per[1], per[2], min_max[1]]
